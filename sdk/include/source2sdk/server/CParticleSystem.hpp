...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_szSnapshotFileName[512]; // 0x7e8            
            // metadata: MNetworkEnable
            bool m_bActive; // 0x9e8            
            // metadata: MNetworkEnable
            bool m_bFrozen; // 0x9e9            
            uint8_t _pad09ea[0x2]; // 0x9ea
            // metadata: MNetworkEnable
            float m_flFreezeTransitionDuration; // 0x9ec            
            // metadata: MNetworkEnable
            std::int32_t m_nStopType; // 0x9f0            
            // metadata: MNetworkEnable
            bool m_bAnimateDuringGameplayPause; // 0x9f4            
            ...
            char m_iEffectIndex[0x8]; // 0x9f8            
            // metadata: MNetworkEnable
            source2sdk::entity2::GameTime_t m_flStartTime; // 0xa00            
            // metadata: MNetworkEnable
            float m_flPreSimTime; // 0xa04            
            // metadata: MNetworkEnable
            Vector m_vServerControlPoints[4]; // 0xa08            
            // metadata: MNetworkEnable
            std::uint8_t m_iServerControlPointAssignments[4]; // 0xa38            
            // metadata: MNetworkEnable
            ...
            char m_hControlPointEnts[0x100]; // 0xa3c            
            // metadata: MNetworkEnable
            bool m_bNoSave; // 0xb3c            
            // metadata: MNetworkEnable
            bool m_bNoFreeze; // 0xb3d            
            // metadata: MNetworkEnable
            bool m_bNoRamp; // 0xb3e            
            bool m_bStartActive; // 0xb3f            
            CUtlSymbolLarge m_iszEffectName; // 0xb40            
            CUtlSymbolLarge m_iszControlPointNames[64]; // 0xb48            
            std::int32_t m_nDataCP; // 0xd48            
            Vector m_vecDataCPValue; // 0xd4c            
            std::int32_t m_nTintCP; // 0xd58            
            Color m_clrTint; // 0xd5c            
        ...
        static_assert(sizeof(source2sdk::server::CParticleSystem) == 0xd60);
    };
};
