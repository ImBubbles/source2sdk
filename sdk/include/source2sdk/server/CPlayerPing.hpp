...
namespace source2sdk
{
    namespace server
    {
        ...
        public:
            ...
            char m_hPingedEntity[0x4]; // 0x4fc            
            // metadata: MNetworkEnable
            std::int32_t m_iType; // 0x500            
            // metadata: MNetworkEnable
            bool m_bUrgent; // 0x504            
            // metadata: MNetworkEnable
            char m_szPlaceName[18]; // 0x505            
            uint8_t _pad0517[0x1];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CPlayerPing because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::server::CPlayerPing) == 0x518);
    };
};
