...
namespace source2sdk
{
    namespace client
    {
        ...
        {
        public:
            bool m_bBeenEnabled; // 0x1120            
            bool m_bHide; // 0x1121            
            uint8_t _pad1122[0x2]; // 0x1122
            float m_flSeenTargetTime; // 0x1124            
            source2sdk::client::C_PointClientUIWorldPanel* m_pTargetPanel; // 0x1128            
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in CPointOffScreenIndicatorUi because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::CPointOffScreenIndicatorUi) == 0x1130);
    };
};
