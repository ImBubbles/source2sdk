...
namespace source2sdk
{
    namespace client
    {
        ...
        public:
            ...
            char m_hPingedEntity[0x4]; // 0x62c            
            // metadata: MNetworkEnable
            std::int32_t m_iType; // 0x630            
            // metadata: MNetworkEnable
            bool m_bUrgent; // 0x634            
            // metadata: MNetworkEnable
            char m_szPlaceName[18]; // 0x635            
            uint8_t _pad0647[0x1];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in C_PlayerPing because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::C_PlayerPing) == 0x648);
    };
};
