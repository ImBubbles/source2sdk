...
namespace source2sdk
{
    namespace client
    {
        ...
        public:
            ...
            std::int32_t m_iActiveIssueIndex; // 0x608            
            // metadata: MNetworkEnable
            std::int32_t m_iOnlyTeamToVote; // 0x60c            
            // metadata: MNetworkEnable
            // metadata: MNetworkChangeCallback "RecvProxy_VoteOption"
            std::int32_t m_nVoteOptionCount[5]; // 0x610            
            // metadata: MNetworkEnable
            std::int32_t m_nPotentialVotes; // 0x624            
            bool m_bVotesDirty; // 0x628            
            bool m_bTypeDirty; // 0x629            
            // metadata: MNetworkEnable
            bool m_bIsYesNoVote; // 0x62a            
            uint8_t _pad062b[0x5];
        };
        #pragma pack(pop)
        
        // Cannot assert offsets of fields in C_VoteController because it is not a standard-layout class
        
        static_assert(sizeof(source2sdk::client::C_VoteController) == 0x630);
    };
};
